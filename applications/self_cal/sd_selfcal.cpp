...
static bool& output_tracks =
    CVarUtils::CreateCVar<>("sd.OutputTracks", false, "");
...
void DoBundleAdjustment()
{
  ...
  //                                 tracker.GetCurrentTracks());
  //  std::cerr << "pre ba level 0 rmse: " << pre_ba_rmse << std::endl;
  ...
  if (current_tracks && poses.size() > 1) {
      ...
      for (std::shared_ptr<sdtrack::DenseTrack> track : pose->tracks) {
        ...
        for (size_t jj = 0; jj < track->keypoints.size() ; ++jj) {
          if (track->keypoints_tracked[jj]) {
            ...
            // std::cerr << "Adding reprojection residual from lm " <<
            //              track->external_id << " to pose " <<  pose->opt_id +
            //              jj << " with z " << z.transpose() << " and rid " <<
            //              res_id << std::endl;
    ...
    for (uint32_t ii = start_pose ; ii < poses.size() ; ++ii) {
      ...
      for (std::shared_ptr<sdtrack::DenseTrack> track: pose->tracks) {
        ...
        // std::cerr << "jtj for track " << track->external_id << " is " <<
        //              track->jtj << std::endl;
        //        std::cerr << "Lm outlier ratio for " << track->external_id <<
        //                     " with " << track->keypoints.size() << "obs. is " <<
        //                     ratio << std::endl;
        if (ratio > 0.4) {
          num_outliers++;
          track->is_outlier = true;
          /*std::cerr << "REJECTING TRACK: " << track->id << " : ";
          for (uint32_t res_id : landmark.proj_residuals) {
            auto proj_residual = bundle_adjuster.GetProjectionResidual(res_id);
            std::cerr << " (" << proj_residual.residual.norm() << "), ";
          }
          std::cerr << " num outliers: " << landmark.num_outlier_residuals <<
  ...
  // std::cerr << "Last_t_ba: " << std::endl << last_t_ba.matrix() << std::endl;

  //  double post_ba_rmse =
  //  tracker.EvaluateTrackResiduals(0, tracker.GetImagePyramid(),
  //                                 tracker.GetCurrentTracks());
  //  std::cerr << "post ba level 0 rmse: " << post_ba_rmse << std::endl;
}
...
void BaAndStartNewLandmarks()
{
  ...
  if (output_tracks) {
    // Create the filename
    if (poses.size() > 0 && current_tracks) {
      char filename[100];
      sprintf(filename, "tracks/%d.txt", poses.size());
      std::ofstream file(filename, std::ios_base::trunc);
      for (std::shared_ptr<sdtrack::DenseTrack>& track : *current_tracks) {
        file << poses.size() << " " << track->id << " " <<
                track->keypoints.back()[0] << " " <<
                track->keypoints.back()[1] << std::endl;
      }
      file.close();
    }
  }
  ...
  // std::cerr << "Adding pose with " << new_pose->tracks.size() << " new "
  //              "tracks and SE3: " << std::endl << new_pose->t_wp.matrix() <<
  //              std::endl;

  //Update the t_ab based on the optimization.
  //std::cerr << "Transforming with delta: " << std::endl <<
  //               tracker.t_ba().matrix() << std::endl;
  if (!do_bundle_adjustment) {
...
void DrawImageData()
{
  ...
  if (handler->selected_track != nullptr) {
    sdtrack::DenseKeypoint& kp = handler->selected_track->ref_keypoint;
