// Copyright (c) University of Colorado Boulder, all rights reserved.  See the
...
std::string g_usage = "SD VOTRACKER. Example usage:\n"
    "-cam file:[loop=1]///Path/To/Dataset/[left,right]*pgm -cmod cameras.xml -odo csv://Path/To/Dataset";
...
hal::WheelOdometry odometry_device;
hal::Poses poses_device;
...
std::vector <hal::WheelOdometryMsg> odometry_buffer;
std::vector <hal::PoseMsg> ground_truth_buffer;
...
//---TOYOTA Specific--- Reads in the ground truth poses
void GroundTruthCallback(const hal::PoseMsg& ref) {

  ground_truth_buffer.push_back(ref);


//  if(ground_truth_buffer.size() % 10 == 0){
//    Sophus::SE3Group<Scalar> pose;

//    hal::PoseMsg msg = ground_truth_buffer.back();
//    hal::ReadPoseSE3(msg, &pose);
//    std::cerr << "\tPushing back new applanix reading ( " << ground_truth_buffer.size() << "): " << std::endl <<
//                  pose.translation() << std::endl;
//  }
}

void GroundTruthFinishedLoadingCallback(){
  std::cerr << "\tLaded " << ground_truth_buffer.size() << " applanix measurements from the csv file." << std::endl;

//    Sophus::SE3Group<double> pose;
//    for (uint ii = 0; ii < 100; ii++){
//      hal::ReadPoseSE3(ground_truth_buffer[ii], &pose);

//      std::cerr << "translation: " << pose.translation().transpose() << std::endl << "rotation:"
//                << pose.rotationMatrix().eulerAngles(0, 1, 2).transpose() << std::endl << "timestamp: "
//                << ground_truth_buffer[ii].device_time() << std::endl << std::endl;
//    }

}


void WheelOdometryCallback(const hal::WheelOdometryMsg& ref) {

  odometry_buffer.push_back(ref);

//  if(odometry_buffer.size() % 100 == 0){
//    hal::WheelOdometryMsg msg = odometry_buffer.back();
//      std::cerr << "\tPushing back new odometry reading(" << odometry_buffer.size() << "): " << std::endl << "\t rl: " <<
//                   msg.rear_left() << std::endl <<"\t rr: " << msg.rear_right() << std::endl <<"\tfl: " <<
//                   msg.front_left() << std::endl << "\tfr: " <<
//                   msg.front_right() << std::endl <<"\ttimestamp: " <<
//                   msg.device_time() << std::endl;
//  }
}

void OdometryFinishedLoadingCallback(){
  std::cerr << "\tLaded " << odometry_buffer.size() << " wheel speed measurements from the csv file." << std::endl;

  std::cerr << "\t Integrating all the wheel speed measurements" << std::endl;
  std::vector<hal::PoseMsg> odometry_poses = odometry_device.IntegrateWheelSpeed(odometry_buffer);
  std::cerr << "\t Finished integrating wheel speed measurements. Got " << odometry_poses.size() << " poses" << std::endl;

//  Sophus::SE3Group<double> pose;
//  int a = 2;
//  for (uint ii = 0; ii < 100; ii++){
//    hal::ReadPoseSE3(odometry_poses[ii], &pose);
//    std::cerr << "translation: " << pose.translation().transpose() << std::endl << "rotation:"
//              << pose.rotationMatrix().eulerAngles(0, 1, 2).transpose() << std::endl << "timestamp: "
//              << odometry_poses[ii].device_time() << std::endl << std::endl;
//  }

  std::cerr << "\t Loading ground truth data..." << std::endl;

  // Load the ground truth (TOYOTA - Applanix)
    std::string poses_str = cl->follow("","-gt");
    if (!poses_str.empty()) {
      try {
        poses_device = hal::Poses(poses_str);
        poses_device.RegisterPosesDataCallback(&GroundTruthCallback);
        poses_device.RegisterPosesFinishedLoadingDataCallback(&GroundTruthFinishedLoadingCallback);
      } catch (hal::DeviceException& e) {
        LOG(ERROR) << "Error loading groud truth poses: " << e.what()
                   << " ... proceeding without.";
      }
    }
}
...
void DoBundleAdjustment(uint32_t num_active_poses, uint32_t id)
{
  ...
  if (current_tracks && poses.size() > 1) {
    ...
    // Add binary constraints to ba (wheel odometry)
//    if(use_odometry_measurements && poses.size() > min_poses_for_odometry){
//      std::shared_ptr<hal::PoseMsg> initial_pose;


//      for (uint32_t ii = start_pose + odometry_window_size ; ii < poses.size() ;
//           ii+=odometry_window_size){

//        // Get the first pose to start the odometry integration at
//        initial_pose->t_wp = poses[ii - odometry_window_size]->t_wp;
//        initial_pose->timestamp = poses[ii - odometry_window_size]->time;

//        // Integrate to the current pose
//        odometry_device.IntegrateWheelSpeed(initial_pose, odometry_buffer, poses[ii]->time - prev_pose->time);
//        Sophus::SE3t t_12 = initial_pose->t_wp;

//        // Add the binary constraint to BA
//        bundle_adjuster.AddBinaryConstraint(poses[ii - odometry_window_size]->opt_id[id], poses[ii]->opt_id[id], t_12);
...
bool LoadCameras()
{
  ...
  // Load the wheel odometry
  if(use_odometry_measurements){
    std::string odometry_str = cl->follow("","-odo");
    if (!odometry_str.empty()) {
      try {
        odometry_device = hal::WheelOdometry(odometry_str);
        odometry_device.RegisterWheelOdometryDataCallback(&WheelOdometryCallback);
        odometry_device.RegisterWheelOdometryFinishedLoadingDataCallback(&OdometryFinishedLoadingCallback);
      } catch (hal::DeviceException& e) {
        LOG(ERROR) << "Error loading wheel odometry device: " << e.what()
                   << " ... proceeding without.";
      }
    }
  }
...
void InitGui()
{
  ...
  pangolin::RegisterKeyPressCallback('o', [&]() {
    odometry_device.IntegrateWheelSpeed(odometry_buffer);
  });
}
