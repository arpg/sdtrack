...
std::string g_usage = "SD VIBATCH. Example usage:\n"
...
std::vector<uint32_t> ba_imu_residual_ids;
                        ...
                        uint32_t id, std::vector<uint32_t>& imu_residual_ids)
{
  ...
  uint32_t end_pose_id =poses.size() - 1;
  start_pose_id = 0;
  start_active_pose = 0;
...
void Run()
{
  ...
  while(!pangolin::ShouldQuit()) {
    ...
    }else if(!poses_optimized && poses.size() > 0){

      // Got all measurements, now do the optimization:
